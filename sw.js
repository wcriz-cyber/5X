// Helius Service Worker v4 — Web Push + Background Notifications
// Maneja: push events (app cerrada), notification clicks, postMessage desde app

const CACHE_NAME = 'helius-v4';

// ── PUSH: recibe notificaciones aunque la app esté cerrada ──────────────
self.addEventListener('push', function(event) {
  let payload = { title: 'Helius', body: '', type: 'info', sound: 'normal', data: {} };
  try {
  ...
  const title = payload.title || 'Helius Trading';
...
self.addEventListener('message', function(event) {
  ...
  event.waitUntil(self.registration.showNotification(d.title || 'Helius', opts));
});
