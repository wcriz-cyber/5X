...
self.addEventListener('activate', event => {
    ...
    // Notificar a la página que el SW está listo
    self.clients.matchAll({ includeUncontrolled: true }).then(clients => {
        clients.forEach(c => c.postMessage({ type: 'SW_READY', version: SW_VERSION }));
    });
});

// ── FETCH — pass-through (no caché agresivo) ─────────────────
self.addEventListener('fetch', event => {
    // Solo interceptar navegación, dejar API calls pasar directo
    if (event.request.mode === 'navigate') {
        event.respondWith(fetch(event.request).catch(() => caches.match('/index.html')));
        return;
    }
    // Dejar pasar todo lo demás (Gate.io API, Supabase, etc.)
});
