...
--    Usado para: slots, botConfig, strategyParams, historial,
...
-- Índice para búsquedas por prefijo (list())
...
CREATE TABLE IF NOT EXISTS support_messages (
  ...
  user_name   TEXT,
  text        TEXT,
  from        TEXT NOT NULL DEFAULT 'user',  -- 'user' | 'admin'
...
-- La app usa el Anon Key directamente (sin auth de usuarios).
-- Política: permitir TODO con Anon Key (la seguridad la maneja el PIN de la app).
-- Para entornos con usuarios autenticados, ajusta estas políticas.
...
-- ═══════════════════════════════════════════════════════════
--
-- El código de la Edge Function está en el archivo:
